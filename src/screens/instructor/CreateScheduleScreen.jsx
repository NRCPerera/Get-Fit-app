...
import { View, Text, StyleSheet, TextInput, TouchableOpacity, Alert, Modal, FlatList, Switch } from 'react-native';
...
const CreateScheduleScreen = () => {
  ...
  const loadClients = useCallback(async () => {
    try {
      ...
      const res = await instructorAPI.getMyClients();
      const payload = res?.data?.clients || res?.data || res;
      setAvailableClients(Array.isArray(payload) ? payload : []);
    } catch (e) {
  ...
  const selectClient = (client) => {
    setAssignedTo(client._id);
    setSelectedClientName(client.name || client.email || 'Client');
  ...
  return (
    <KeyboardAvoidingWrapper style={[styles.container, { backgroundColor: colors.background }]} contentContainerStyle={styles.content}>
      <View style={styles.header}>
        ...
        <Text style={[styles.subtitle, { color: colors.textSecondary }]}>Build workout plans for your clients</Text>
      </View>
      ...
      <View style={styles.section}>
        ...
        <View style={styles.inputGroup}>
          ...
          <View style={styles.optionRow}>
            {SCHEDULE_TYPES.map(type => (
              <TouchableOpacity
                key={type}
                style={[styles.optionButton, { backgroundColor: colors.backgroundSecondary }, scheduleType === type && { backgroundColor: colors.primary }]}
                onPress={() => setScheduleType(type)}
              >
                <Text style={[styles.optionText, { color: colors.textSecondary }, scheduleType === type && { color: colors.white }]}>
        ...
        <View style={styles.inputGroup}>
          ...
          <TextInput
            style={[styles.input, { height: 80, textAlignVertical: 'top', backgroundColor: colors.surface, borderColor: colors.border, color: colors.text }]}
            value={description}
        ...
        <View style={styles.inputGroup}>
          ...
          <View style={styles.optionRow}>
            {DIFFICULTY_OPTIONS.map(opt => (
              <TouchableOpacity
                key={opt}
                style={[styles.optionButton, { backgroundColor: colors.backgroundSecondary }, difficulty === opt && { backgroundColor: colors.primary }]}
                onPress={() => setDifficulty(difficulty === opt ? '' : opt)}
              >
                <Text style={[styles.optionText, { color: colors.textSecondary }, difficulty === opt && { color: colors.white }]}>
        ...
        <View style={styles.inputGroup}>
          ...
          <View style={styles.goalsContainer}>
            {GOAL_OPTIONS.map(goal => (
              <TouchableOpacity
                key={goal}
                style={[styles.goalButton, { backgroundColor: colors.backgroundSecondary, borderColor: colors.border }, goals.includes(goal) && { backgroundColor: colors.primary + '20', borderColor: colors.primary }]}
                onPress={() => toggleGoal(goal)}
              >
                <Text style={[styles.goalText, { color: colors.text }, goals.includes(goal) && { color: colors.primary, fontWeight: theme.typography.fontWeight.semibold }]}>
                  {goal}
                </Text>
              </TouchableOpacity>
            ))}
      ...
      <View style={styles.section}>
        <Text style={[styles.sectionTitle, { color: colors.text }]}>Assignment & Template</Text>
        <View style={styles.inputGroup}>
          <View style={styles.switchRow}>
            <View style={styles.switchLabel}>
              <Ionicons name="document-text-outline" size={20} color={colors.textSecondary} />
              <Text style={[styles.switchText, { color: colors.text }]}>Save as Template</Text>
            </View>
            <Switch
              value={isTemplate}
          ...
          <View style={styles.inputGroup}>
            ...
            >
              <Text style={[styles.selectButtonText, { color: colors.text }]}>
                {assignedTo ? selectedClientName : 'Select Client'}
              </Text>
        ...
        {scheduleType !== '1-day' && (
          <View style={styles.dayTabs}>
            {Array.from({ length: scheduleType === '2-day' ? 2 : 3 }, (_, i) => i + 1).map(day => (
              <TouchableOpacity
                key={day}
                style={[styles.dayTab, { backgroundColor: colors.backgroundSecondary }, selectedDay === day && { backgroundColor: colors.primary }]}
                onPress={() => setSelectedDay(day)}
              >
                <Text style={[styles.dayTabText, { color: colors.textSecondary }, selectedDay === day && { color: colors.white }]}>
                  Day {day}
                </Text>
              </TouchableOpacity>
            ))}
      ...
      <View style={styles.section}>
        ...
        <TextInput
          style={[styles.input, { height: 100, textAlignVertical: 'top', backgroundColor: colors.surface, borderColor: colors.border, color: colors.text }]}
          value={notes}
      ...
      <TouchableOpacity
        onPress={onSave}
        disabled={!canSave}
        style={[styles.saveButton, { backgroundColor: colors.primary }, !canSave && styles.saveButtonDisabled]}
      >
        ...
        <View style={[styles.modalContainer, { backgroundColor: colors.background }]}>
          ...
          {loadingExercises ? (
            ...
            <FlatList
              ...
              ListEmptyComponent={<Text style={[styles.emptyText, { color: colors.textSecondary }]}>No exercises available</Text>}
            />
          )}
        </View>
      </Modal>
      ...
      >
        <View style={[styles.modalContainer, { backgroundColor: colors.background }]}>
          ...
          {loadingClients ? (
            <View style={styles.modalLoading}>
              <Text style={{ color: colors.textSecondary }}>Loading clients...</Text>
            </View>
          ) : (
            <FlatList
              ...
              renderItem={({ item }) => (
                ...
                >
                  <Text style={[styles.exerciseListItemName, { color: colors.text }]}>{item.name || item.email}</Text>
                  <Text style={[styles.exerciseListItemMeta, { color: colors.textSecondary }]}>{item.email}</Text>
                </TouchableOpacity>
              )}
              ListEmptyComponent={<Text style={[styles.emptyText, { color: colors.textSecondary }]}>No clients available</Text>}
            />
          )}
        </View>
      </Modal>
...
const styles = StyleSheet.create({
  ...
  switchLabel: { flexDirection: 'row', alignItems: 'center', gap: theme.spacing.sm },
  switchText: { fontSize: theme.typography.fontSize.md },
  selectButton: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', borderRadius: theme.borderRadius.lg, padding: theme.spacing.md, borderWidth: 1 },
  selectButtonText: { fontSize: theme.typography.fontSize.md },
